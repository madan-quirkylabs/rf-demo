...
export const sebiSIDCircular: Circular = {
  ...
  /** Auto-derived checklist — practitioner 6-header categorization. */
  checklist: [
    {
      id: 'CHK-S1',
      ref: 'Para 10.1',
      section: 'Offer Document — Scheme Information Document',
      action: 'Prepare each Scheme Information Document in the simplified, uniform format prescribed by SEBI.',
      appliesTo: ['Mutual Funds', 'AMCs'],
      category: 'actionable',
      deadline: { kind: 'event', trigger: 'On every new SID / SID revision', leadTime: 'Before filing the offer document' },
      evidenceExpected: 'SID mapped section-by-section onto the simplified template; sign-off by Chief Compliance Officer / Trustees.',
    },
    {
      id: 'CHK-S2',
      ref: 'Circular 2024/179',
      section: 'Simplification of Offer Documents',
      action: 'Adopt the new format for all existing schemes within the prescribed transition window.',
      appliesTo: ['Mutual Funds', 'AMCs'],
      category: 'event',
      deadline: { kind: 'fixed', date: 'Transition window per AMFI guidance — ~6 months from circular date' },
      evidenceExpected: 'Scheme-wise adoption tracker showing old→new format migration status and completion date.',
    },
    {
      id: 'CHK-S3',
      ref: 'Para 10.1',
      section: 'Offer Document — Scheme Information Document',
      action: 'Facilitate periodic updation of the SID; surface what changed relative to the prior version at each updation.',
      appliesTo: ['Mutual Funds', 'AMCs'],
      category: 'reporting',
      deadline: { kind: 'periodic', frequency: 'Each periodic updation' },
      evidenceExpected: 'Change-log annexure listing amendments vs prior SID, referenced clause-by-clause.',
    },
    {
      id: 'CHK-S4',
      ref: 'Para 10',
      section: 'Easy Availability of Offer Document',
      action: 'Ensure the current SID and SAI are readily available with all distributors and ISCs.',
      appliesTo: ['Mutual Funds', 'AMCs', 'Distributors / ISCs'],
      category: 'policy',
      deadline: { kind: 'none', note: 'Ongoing' },
      evidenceExpected: 'Distribution-availability confirmations / portal checks evidencing current documents with each ISC.',
    },
    {
      id: 'CHK-S5',
      ref: 'Para 10',
      section: 'Easy Availability of Offer Document',
      action: 'Confirm SID/SAI availability to SEBI through the half-yearly trustee report.',
      appliesTo: ['Trustees', 'AMCs'],
      category: 'reporting',
      deadline: { kind: 'periodic', frequency: 'Half-yearly' },
      evidenceExpected: 'Half-yearly trustee report excerpt containing the availability confirmation.',
    },
    {
      id: 'CHK-S6',
      ref: 'AMFI Guidance Note',
      section: 'Industry implementation guidance (non-binding)',
      action: 'Note: AMFI recommends phasing adoption equity-schemes-first; regulatory deadline remains firm for all schemes.',
      appliesTo: ['Mutual Funds', 'AMCs'],
      category: 'info',
      deadline: { kind: 'none', note: 'No action — planning input only' },
      evidenceExpected: 'Not applicable — industry guidance informing internal sequencing decisions.',
    },
  ],
};

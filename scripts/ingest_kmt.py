...
STOPWORDS = {
    "amendment", "amended", "second", "circular", "directions", "master",
    "notification", "guideline", "the", "and", "of", "on", "to", "in", "for",
    "vide", "dated", "with", "a", "an", "regarding", "issued",
}


def title_tokens(t: str):
    return {w for w in re.findall(r"[a-z]{4,}", t.lower()) if w not in STOPWORDS}


def subject_of(t: str):
    """The parenthetical subject, e.g. '(Commercial Banks - Responsible
    Business Conduct) Directions' → 'commercial banks - responsible business
    conduct'. None when the title has no parenthetical."""
    m = re.search(r"\(([^)]+)\)", t)
    return re.sub(r"\s+", " ", m.group(1)).strip().lower() if m else None


def find_prior(title: str, authority: str, after: date, pool):
    """Most recent same-authority document with the same subject. Primary
    rule: identical parenthetical subject (the RBI/IFSCA amendment pattern).
    Fallback (no parenthetical): strong title-token overlap only."""
    subj = subject_of(title)
    toks = title_tokens(title)
    best = None  # (date, row)
    for d, r in pool:
        if r[0] != authority or d >= after or not r[5]:
            continue
        if subj:
            if subject_of(r[3]) == subj:
                if best is None or d > best[0]:
                    best = (d, r)
        elif toks:
            overlap = toks & title_tokens(r[3])
            if len(overlap) >= max(3, (len(toks) * 3) // 4):
                if best is None or d > best[0]:
                    best = (d, r)
    return best


def pair_amendments(out, pool):
    """For amendment circulars whose prior version exists in the corpus, set
    per-para previousText + changeType (amended/new) — the Devesh diff."""
    paired = 0
    for entry in out:
        if not re.search(r"amend", entry["title"], re.I):
            continue
        prior = find_prior(entry["title"], entry["regulator"], parse_doc_date(
            datetime.strptime(entry["dated"], "%d %b %Y").strftime("%d-%b-%Y")), pool)
        if not prior:
            continue
        d, (authority, ref_no, doc_date, ptitle, ctype, ptext) = prior
        prior_paras = {int(m.group(1)): body for _s, m_ref, body in split_paragraphs(ptext)
                       for m in [re.match(r"Para (\d+)$", m_ref)] if m}
        if not prior_paras:
            continue
        n_amended = n_new = 0
        for cl in entry["clauses"]:
            n = int(re.match(r"Para (\d+)$", cl["ref"]).group(1))
            old = prior_paras.get(n)
            if old and old.strip() != cl["text"].strip():
                cl["previousText"] = old
                ...
                cl["changeType"] = "new"
                n_new += 1
        if n_amended or n_new:
            paired += 1
            entry["firstTracked"] = False
            entry["changeDiff"] = {
                "previous": [f"Prior version: {ptitle.strip()} (dated {doc_date})"],
                "current": [f"{n_amended} paras amended and {n_new} paras added vs. the prior version"],
            }
            entry["keyChange"] = {"headline": "Amendment", "detail": ""}
    return paired
...
def main():
    ...
    paired = pair_amendments(out, dated)
    ...
    print(f"amendment-paired circulars: {paired}")
